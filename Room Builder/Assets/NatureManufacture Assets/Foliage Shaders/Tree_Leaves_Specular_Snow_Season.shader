Shader "NatureManufacture Shaders/Trees/Tree Leaves Specular Snow Season"
{
	Properties
	{
		_Cutoff( "Mask Clip Value", Float ) = 0.5
		_Season("Season", Range( 0 , 4)) = 2.783087
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 appendResult305 = (float2(ase_worldPos.x , ase_worldPos.z));
			...
			float4 temp_output_97_0 = ( lerpResult310 * lerpResult108 );
			float2 uv_SnowAlbedoRGB = i.uv_texcoord * _SnowAlbedoRGB_ST.xy + _SnowAlbedoRGB_ST.zw;
			float3 appendResult112 = (float3(_SnowBrightnessReduction , _SnowBrightnessReduction , _SnowBrightnessReduction));
			...
			float clampResult313 = clamp( _Snow_Amount , 0.1 , 2.0 );
			float lerpResult316 = lerp( 0.0 , clampResult93 , pow( tex2DNode3.a , ( _SnowMaskTreshold / clampResult313 ) ));
			float4 lerpResult51 = lerp( temp_output_97_0 , ( tex2D( _SnowAlbedoRGB, uv_SnowAlbedoRGB ) - float4( appendResult112 , 0.0 ) ) , lerpResult316);
			...
			float4 tex2DNode98 = tex2D( _AmbientOcclusionGSmoothnessA, uv_MainTex );
			...
			float clampResult114 = clamp( tex2DNode98.g , ( 1.0 - _AmbientOcclusionPower ) , 1.0 );
			float clampResult116 = clamp( tex2D( _SnowAmbientOcclusionG, uv_SnowAlbedoRGB ).g , ( 1.0 - _SnowAmbientOcclusionPower ) , 1.0 );
